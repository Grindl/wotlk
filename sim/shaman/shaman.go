...
func NewShaman(character *core.Character, talents string, totems *proto.ShamanTotems, selfBuffs SelfBuffs, thunderstormRange bool) *Shaman {
	...
	if shaman.Totems.Fire == proto.FireTotem_TotemOfWrath && true {
	...
	shaman.AddStatDependency(stats.Agility, stats.MeleeCrit, core.CritPerAgiAtLevel[character.Class][int(character.Level)]*core.CritRatingPerCritChance)
...
func (shaman *Shaman) ElementalCritMultiplier(secondary float64) float64 {
	critBonus := core.TernaryFloat64(shaman.Talents.ElementalFury, 1.0, 0.0) + secondary
